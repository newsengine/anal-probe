...
const RULES: { sink: string; re: RegExp }[] = [
...
export function scanDomXssSinks(js: string): { sink: string; snippet: string }[] {
  const hits: { sink: string; snippet: string }[] = [];
  for (const r of RULES) {
    const m = js.match(r.re);
    if (m) hits.push({ sink: r.sink, snippet: m[0].replace(/\s+/g, ' ').trim().slice(0, 100) });
  }
  return hits;
}

export function domXssFindings(ctx: ScanContext): Finding[] {
  if (!ctx.html) return [];
  const inlineScripts = [...ctx.html.matchAll(/<script\b(?![^>]*\bsrc=)[^>]*>([\s\S]*?)<\/script>/gi)].map((m) => m[1]).join('\n');
  const hits = scanDomXssSinks(inlineScripts);
  if (!hits.length) return []; // stay silent unless a direct flow is found (low-FP)
  const uniq = [...new Map(hits.map((h) => [h.sink, h])).values()];
  return [f('dom-xss', `Potential DOM XSS: user input reaches ${uniq.length} sink(s)`, 'medium', false,
    uniq.map((h) => `${h.sink} ← "${h.snippet}"`).join(' | ') + ' — a URL/referrer/window.name value flows into an HTML/JS sink in inline script',
    'Never write untrusted URL data into innerHTML/document.write/eval. Use textContent, or sanitize with a library like DOMPurify before inserting HTML.')];
}
