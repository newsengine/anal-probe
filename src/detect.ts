...
// `strong`: a single distinctive signal that alone justifies HIGH confidence (e.g. __NEXT_DATA__).
// A lone weight-2 but NON-strong signal (like the spoofable `x-powered-by`) only reaches MEDIUM.
interface Rule { name: StackName; signals: { weight: 2 | 1; strong?: boolean; why: string; test: (i: DetectInput, h: (n: string) => string) => boolean }[] }

const RULES: Rule[] = [
  { name: 'Next.js', signals: [
    { weight: 2, strong: true, why: '__NEXT_DATA__ in HTML', test: (i) => /id="__NEXT_DATA__"|__NEXT_DATA__\s*=/.test(i.html) },
  ...
  { name: 'Nuxt', signals: [
    { weight: 2, strong: true, why: '__NUXT__ in HTML', test: (i) => /window\.__NUXT__|id="__nuxt"/.test(i.html) },
    { weight: 1, why: '/_nuxt/ asset references', test: (i) => /\/_nuxt\//.test(i.html) },
  ] },
  { name: 'WordPress', signals: [
    { weight: 2, strong: true, why: 'generator meta = WordPress', test: (i) => /<meta[^>]+name=["']generator["'][^>]+WordPress/i.test(i.html) },
  ...
  { name: 'Laravel', signals: [
    { weight: 2, strong: true, why: 'laravel_session cookie', test: (i) => i.setCookie.some((c) => /^laravel_session=/i.test(c)) },
    { weight: 1, why: 'XSRF-TOKEN cookie', test: (i) => i.setCookie.some((c) => /^XSRF-TOKEN=/i.test(c)) },
  ] },
  { name: 'Django', signals: [
    { weight: 2, strong: true, why: 'csrftoken + sessionid cookies', test: (i) => i.setCookie.some((c) => /^csrftoken=/i.test(c)) && i.setCookie.some((c) => /^sessionid=/i.test(c)) },
    { weight: 1, why: 'csrftoken cookie', test: (i) => i.setCookie.some((c) => /^csrftoken=/i.test(c)) },
  ] },
  { name: 'Rails', signals: [
    { weight: 2, strong: true, why: 'x-runtime header + _session cookie', test: (i, h) => !!h('x-runtime') && i.setCookie.some((c) => /_session=/i.test(c)) },
  ...
  { name: 'Spring Boot', signals: [
    { weight: 2, strong: true, why: 'X-Application-Context header', test: (_i, h) => !!h('x-application-context') },
    { weight: 1, why: 'JSESSIONID cookie', test: (i) => i.setCookie.some((c) => /^JSESSIONID=/i.test(c)) },
  ] },
  { name: 'ASP.NET', signals: [
    { weight: 2, strong: true, why: 'X-AspNet-Version header', test: (_i, h) => !!h('x-aspnet-version') },
...
export function detectStacks(inp: DetectInput): StackSignal[] {
  ...
  for (const rule of RULES) {
    ...
    // HIGH needs corroboration: a distinctive `strong` signal, OR two independent signals. A lone
    // weight-2-but-spoofable header (x-powered-by) only reaches MEDIUM, so its checks won't run.
    const confidence: StackSignal['confidence'] = hit.some((s) => s.strong) || hit.length >= 2 ? 'high' : 'medium';
    out.push({ name: rule.name, confidence, why: hit.map((s) => s.why) });
  }
