...
export type AppStyleKey =
  | 'ecommerce' | 'saas-dashboard' | 'marketing-site' | 'blog-cms' | 'auth-portal'
  | 'api-backend' | 'marketplace' | 'booking' | 'fintech' | 'healthcare'
  | 'social-community' | 'chat-messaging' | 'file-sharing' | 'admin-panel' | 'docs-site'
  | 'job-board' | 'real-estate' | 'education-lms' | 'crm' | 'support-helpdesk';

export interface AppStyleSignal { key: AppStyleKey; label: string; score: number; strong: boolean; why: string[] }

/** Pre-computed, cheap views of the homepage every detector/rule shares. */
export interface AppContext {
  html: string;
  hay: string;   // lowercased raw HTML
  text: string;  // lowercased visible-ish text (tags stripped)
  path: string;  // lowercased URL path
  jsonLdTypes: string[]; // lowercased @type values
  headers: Headers;
  isHttps: boolean;
}
...
function buildAppContext(ctx: ScanContext): AppContext {
  ...
  for (const m of html.matchAll(/<script[^>]+type=["']application\/ld\+json["'][^>]*>([\s\S]*?)<\/script>/gi)) {
    for (const t of m[1].matchAll(/"@type"\s*:\s*"([^"]+)"/g)) jsonLdTypes.push(t[1].toLowerCase());
  }
...
const kw = (a: AppContext, ...words: string[]) => words.filter((w) => new RegExp(`\\b${w.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`).test(a.text));

interface Detector { key: AppStyleKey; label: string; signals: { weight: number; why: string; test: (a: AppContext) => boolean }[] }
...
export function detectAppStyles(ctx: ScanContext): AppStyleSignal[] {
  const a = buildAppContext(ctx);
  const out: AppStyleSignal[] = [];
  for (const d of DETECTORS) {
    let score = 0; let strong = false; const why: string[] = [];
    for (const s of d.signals) { if (s.test(a)) { score += s.weight; why.push(s.why); if (s.weight >= 3) strong = true; } }
    if (score > 0) out.push({ key: d.key, label: d.label, score, strong, why });
  }
  return out.sort((x, y) => y.score - x.score);
}

// ── the archetype rule packs ─────────────────────────────────────────────────────────────────────
// A rule returns a Finding, or null when the check doesn't apply to this particular page (e.g. no form
// to look at). Rules are black-box: they read the shared homepage response and at most make a few safe
// GETs. Severity is contextual to the archetype (a missing CSP matters more on a store than a docs site).

interface AppRule {
  id: string; title: string; severity: Severity; description: string;
  run: (ctx: ScanContext, a: AppContext) => Promise<{ pass: boolean; detail: string; fix?: string } | null> | ({ pass: boolean; detail: string; fix?: string } | null);
}
interface AppStylePack { key: AppStyleKey; rules: AppRule[] }

const has = (a: AppContext, name: string) => !!a.headers.get(name);
const cspPresent = (a: AppContext) => has(a, 'content-security-policy');
const hstsPresent = (a: AppContext) => /max-age=\d{4,}/.test(a.headers.get('strict-transport-security') || '');
const frameProtected = (a: AppContext) => has(a, 'x-frame-options') || /frame-ancestors/i.test(a.headers.get('content-security-policy') || '');
const noStore = (a: AppContext) => /no-store|private/i.test(a.headers.get('cache-control') || '');
function crossOriginScriptsNoSri(a: AppContext, base: string): number {
  ...
  { key: 'auth-portal', rules: [
    ...
    { id: 'appstyle.auth.form-secure', title: 'Login form posts to an https target', severity: 'high', description: 'A login <form> whose action is http (or method GET) leaks credentials.',
      run: (_c, a) => {
        ...
        if (!/<input[^>]+type=["']password["']/i.test(a.html)) return null;
    ...
    { id: 'appstyle.api.docs-public', title: 'API docs/spec are not wide open', severity: 'medium', description: 'A public OpenAPI/Swagger spec hands attackers your full endpoint map.',
      run: async (c, _a) => {
        for (const p of ['/openapi.json', '/swagger.json', '/api-docs']) {
          ...
          if (res && res.ok) { const b = (await res.text().catch(() => '')).slice(0, 2000); if (/"(?:openapi|swagger)"\s*:|swagger-ui/i.test(b)) return { pass: false, detail: `${p} serves an API spec publicly`, fix: 'Gate the OpenAPI/Swagger spec behind auth (or don\'t ship it to production) so your endpoint map isn\'t public.' }; }
        }
  ...
  { key: 'admin-panel', rules: [
    { id: 'appstyle.admin.noindex', title: 'Admin panel is not search-indexable', severity: 'low', description: 'An admin panel showing up in search advertises the attack surface.',
      run: async (c, a) => {
        ...
        if (metaNoindex) return { pass: true, detail: 'admin marked noindex' };
...
const LABELS: Record<AppStyleKey, string> = Object.fromEntries(DETECTORS.map((d) => [d.key, d.label])) as Record<AppStyleKey, string>;

const f = (id: string, title: string, severity: Severity, pass: boolean, detail: string, fix?: string): Finding =>
  ({ category: 'appstyle', id, title, severity, pass, detail, fix });
 ...
 */
export async function appStyleChecks(ctx: ScanContext): Promise<Finding[]> {
  if (!ctx.res) return [];
  const a = buildAppContext(ctx);
  ...
  if (!confident.length) {
    return [f('appstyle.none', 'App type not confidently identified', 'info', true,
      styles.length ? `low-confidence guesses: ${styles.slice(0, 3).map((s) => `${s.key} (${s.score})`).join(', ')}` : 'no distinctive app-type markers on the homepage')];
  }

  const out: Finding[] = [f('appstyle.detected', `App type: ${confident.map((s) => s.label).join(', ')}`, 'info', true,
    confident.map((s) => `${s.key} [score ${s.score}: ${s.why.join('; ')}]`).join(' | '))];

  for (const s of confident) {
    const pack = PACKS.find((p) => p.key === s.key);
    if (!pack) continue;
    for (const rule of pack.rules) {
      let res: { pass: boolean; detail: string; fix?: string } | null = null;
      try { res = await rule.run(ctx, a); } catch { res = null; }
      if (!res) continue;
...
// ── catalog integration: expose a flat spec of every archetype rule so docs/CHECKS.md lists them all ──
export interface AppStyleRuleSpec { id: string; key: AppStyleKey; label: string; title: string; severity: Severity; description: string }

export function appStyleRuleSpecs(): AppStyleRuleSpec[] {
  const specs: AppStyleRuleSpec[] = [];
