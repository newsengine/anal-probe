...
export interface PortResult { port: number; open: boolean; service?: ServiceInfo; banner?: string; product?: string; version?: string }
 ...
 */
export function identifyBanner(banner: string): { product?: string; version?: string } {
  if (!banner) return {};
  ...
  if (m) return { product: m[1], version: m[2] || undefined };
  ...
  await Promise.all(open.map(async (p) => {
    if (!p.banner && HTTP_PORTS.has(p.port)) {
      ...
      try {
        ...
        if (server) p.banner = `Server: ${server}`;
