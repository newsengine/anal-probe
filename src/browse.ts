...
export async function browseChecks(startUrl: string, opts: { pages?: number; timeoutMs?: number; headers?: Record<string, string> } = {}): Promise<Finding[]> {
  ...
  catch { return [f('browse.missing-dep', 'browser mode needs playwright-core', 'info', true, 'run `npm i -D playwright-core` to enable `vibetesting-agent browse` (uses your installed Chrome)')]; }
