...
export type CheckClass =
  | 'passive'        // derived from the single homepage response (headers/HTML); no extra requests
  | 'probe'          // makes extra safe, same-origin GET requests (dotfiles, well-known paths, assets)
  | 'active'         // sends a crafted payload / burst / spoofed header / write — louder than a plain GET
  | 'authenticated'  // only meaningful when scanning behind login (--cookie / --header)
  | 'white-box';     // testkit helper: needs source, tokens, or two accounts — not part of the URL scan
...
export const CATALOG: CheckSpec[] = [
  // ── security: transport ──────────────────────────────────────────────────────────────────────
  ...
  // ── security: active injection engine (#43 — opt-in --authorized-active, non-destructive) ────────
  { id: 'inject.sqli', dynamic: true, category: 'security', title: 'SQL injection', severity: 'high', cls: 'active', optIn: true, since: '0.7.0', description: 'A GET parameter is injectable via error/boolean/time-based SQL injection (--authorized-active).' },
  { id: 'inject.ssti', dynamic: true, category: 'security', title: 'Server-side template injection', severity: 'high', cls: 'active', optIn: true, since: '0.7.0', description: 'A GET parameter renders as a server-side template (7*7→49) (--authorized-active).' },
  { id: 'inject.cmdi', dynamic: true, category: 'security', title: 'OS command injection', severity: 'high', cls: 'active', optIn: true, since: '0.7.0', description: 'A GET parameter injects into a shell (time-based) (--authorized-active).' },
  { id: 'inject.traversal', dynamic: true, category: 'security', title: 'Path traversal', severity: 'high', cls: 'active', optIn: true, since: '0.7.0', description: 'A GET parameter allows ../ path traversal (/etc/passwd) (--authorized-active).' },
  { id: 'inject.none', category: 'security', title: 'No injection on tested params', severity: 'info', cls: 'active', optIn: true, since: '0.7.0', description: 'Clean-signal pass — active injection probes found nothing on the tested parameters.' },
...
for (const spec of atlasCheckSpecs()) {
  const cls: CheckClass = spec.optIn ? 'active' : spec.id === 'atlas.detected' || spec.id === 'atlas.none' ? 'passive' : 'probe';
  CATALOG.push({
...
export function catalogEntryFor(id: string): CheckSpec | undefined {
  ...
  let best: CheckSpec | undefined;
  for (const c of CATALOG) {
    if (c.dynamic && id.startsWith(c.id) && (!best || c.id.length > best.id.length)) best = c;
  }
  return best;
}

/** Every id/family the scanner (non-testkit) is expected to be able to emit. */
export function scannerSpecs(): CheckSpec[] {
...
export function vtaNumber(id: string): number | undefined {
  if (id in CATALOG_NUMBERS) return CATALOG_NUMBERS[id];
...
export function vtaCode(id: string): string {
 ...
 */
export function assignNumbers(ids: string[], existing: Record<string, number> = CATALOG_NUMBERS): Record<string, number> {
  const out: Record<string, number> = { ...existing };
  let next = Object.values(out).reduce((m, n) => Math.max(m, n), 0) + 1;
  for (const id of ids) if (!(id in out)) out[id] = next++;
  return out;
}

/** Serialize a numbering registry to the exact contents of src/catalog-numbers.ts (deterministic). */
export function renderCatalogNumbers(map: Record<string, number>): string {
...
const CLASS_LABEL: Record<CheckClass, string> = {
...
export function renderCatalogMarkdown(): string {
  ...
  lines.push('');
  for (const k of Object.keys(CLASS_LABEL) as CheckClass[]) lines.push(`- **${k}** — ${CLASS_LABEL[k].replace(/^[a-z-]+ \(/, '').replace(/\)$/, '')}`);
  lines.push('');
...
function standardsFor(c: CheckSpec): string {
  const r = refsFor(c.dynamic ? sampleIdFor(c.id) : c.id);
  const bits: string[] = [];
  if (r.owasp) bits.push(r.owasp);
  if (r.apiTop10) bits.push(r.apiTop10);
  if (r.cwe?.length) bits.push(r.cwe.join(' ')); // cwe values already carry the "CWE-" prefix
  if (c.atlas?.length) bits.push(c.atlas.join(' '));
  return bits.join('; ');
}

/** For a dynamic family, use a representative concrete id so refsFor resolves. */
function sampleIdFor(id: string): string {
  switch (id) {
