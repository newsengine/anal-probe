...
<title>vibetesting-agent report — ${esc(url)}</title>
<style>
...
<footer>Generated by vibetesting-agent · black-box scan (no source, no attack payloads). Findings are signature-validated to minimise false positives; verify high-severity items in context before remediation. Standards mapping is informational.</footer>
</div></body></html>`;
}
