...
function stdBadges(id: string): string {
  const r = refsFor(id);
  const parts: string[] = [];
  if (r.owasp) parts.push(r.owasp);
  if (r.asvs?.length) parts.push(r.asvs[0]);
  if (r.cwe?.length) parts.push(r.cwe[0]);
...
export function renderReport(url: string, findings: Finding[], opts: { generatedAt?: string } = {}): string {
  ...
  for (const f of failed) if (f.severity in counts) counts[f.severity]++;
  const grade = securityGrade(findings);
  const tls = tlsGrade(findings);
  const cats = [...new Set(findings.map((f) => f.category))];

  const rows = (cat: string) => findings
  ...
  const asvsN = (s: string) => asvs.filter((r) => r.status === s).length;
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .muted{color:#aab}
  .asvs{display:grid;grid-template-columns:repeat(auto-fill,minmax(230px,1fr));gap:5px;font-size:12.5px;
    font-family:ui-monospace,monospace}
  .asvs div{padding:4px 6px}
  .asvs .pass{color:var(--ok)} .asvs .fail{color:var(--hi)} .asvs .no{color:#99a} .asvs .nc{color:#bbc}
...
<section>
  ...
  <div class="asvs">${asvs.map((r) => `<div class="${r.status === 'pass' ? 'pass' : r.status === 'fail' ? 'fail' : r.status === 'not-observed' ? 'no' : 'nc'}">${r.status === 'pass' ? '✓' : r.status === 'fail' ? '✗' : '·'} ${esc(r.id)} ${esc(r.text)}</div>`).join('')}</div>
  ${Object.keys(top10).length ? `<h2>OWASP Top 10 (2021) — open issue categories</h2><div class="asvs">${Object.keys(top10).sort().map((k) => `<div class="fail">${k} ${esc(OWASP_TOP10_NAMES[k])} — ${top10[k]}</div>`).join('')}</div>` : ''}
</section>
