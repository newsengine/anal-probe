...
export async function hostHeaderChecks(ctx: ScanContext): Promise<Finding[]> {
  ...
  if (inRedirect || inBody) {
    return [f('host-header.injection', 'Host header is reflected into URLs (Host-header injection)', 'medium', false,
      `a spoofed X-Forwarded-Host (${MARKER}) was reflected into ${inRedirect ? 'the redirect Location' : 'an absolute URL in the response'} — this enables password-reset-link poisoning and web-cache poisoning`,
      'Never build absolute URLs from the incoming Host/X-Forwarded-Host. Use a configured canonical base URL, and validate Host against an allow-list at the edge.')];
  }
