...
export interface JwtAnalysis {
  ...
  hasExp: boolean;
  expSeconds?: number;
  lifetimeDays?: number;   // exp - iat, when both present
...
export function analyzeJwt(token: string): JwtAnalysis {
  ...
  if (parts.length !== 3) return { valid: false, hasExp: false, sensitiveClaims: [] };
  let header: any = null, claims: any = null;
  try { header = JSON.parse(b64urlDecode(parts[0])); } catch { /* not a JWT */ }
  try { claims = JSON.parse(b64urlDecode(parts[1])); } catch { /* opaque payload */ }
  if (!header || typeof header !== 'object') return { valid: false, hasExp: false, sensitiveClaims: [] };
...
function tokenInNonHttpOnlyCookie(token: string, setCookies: string[]): boolean {
  return setCookies.some((sc) => sc.includes(token.slice(0, 24)) && !/;\s*httponly/i.test(sc));
}

export function jwtFindings(ctx: ScanContext): Finding[] {
  const out: Finding[] = [];
  // Gather candidate tokens from headers + document.
  const setCookies: string[] = typeof (ctx.headers as any).getSetCookie === 'function'
    ? (ctx.headers as any).getSetCookie()
    : (ctx.headers.get('set-cookie') ? [ctx.headers.get('set-cookie') as string] : []);
  ...
  if (!tokens.length) return out; // JWTs are optional; stay silent when none are exposed to the browser
  ...
  if (!anyIssue) out.push(f('jwt.ok', 'Exposed JWT(s) look hygienic', 'info', true, `${tokens.length} JWT(s) seen; signed with an explicit alg, all have exp, none over-long`, undefined));
  return out;
}
