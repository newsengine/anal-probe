...
// Used as `security-kit audit [--prod] [--level low|moderate|high|critical]` to gate CI on vuln deps.
import { execFile } from 'node:child_process';

export interface AuditResult {
  counts: Record<string, number>; // info/low/moderate/high/critical
  total: number;
  raw?: unknown;
  error?: string;
}

export function runNpmAudit(cwd = process.cwd(), prodOnly = false): Promise<AuditResult> {
  return new Promise((resolve) => {
    ...
    execFile('npm', args, { cwd, maxBuffer: 64 * 1024 * 1024 }, (_err, stdout) => {
      try {
        ...
        resolve({ counts: {}, total: 0, error: `could not parse npm audit output: ${String((e as any)?.message || e)}` });
      }
    });
  });
}
...
export function failsAtLevel(result: AuditResult, level: string): boolean {
  const idx = ORDER.indexOf(level);
  if (idx < 0) return result.total > 0;
