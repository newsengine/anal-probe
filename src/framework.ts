...
import { detectStacks, type StackName } from './detect.js';
...
const f = (id: string, title: string, severity: Severity, pass: boolean, detail: string, fix?: string): Finding =>
  ({ category: 'framework', id, title, severity, pass, detail, fix });

/** A reachable path whose BODY confirms a real exposure (not an SPA/404 fallback). */
interface PathProbe { path: string; severity: Severity; label: string; looksReal: (body: string, ct: string) => boolean; fix: string }

async function probePaths(ctx: ScanContext, probes: PathProbe[]): Promise<Finding[]> {
  const out: Finding[] = [];
...
const CHECKS: Partial<Record<StackName, (ctx: ScanContext) => Promise<Finding[]>>> = {
  'Next.js': async (ctx) => {
    const out: Finding[] = [];
    ...
    out.push(f('framework.next.data-secrets', leaked.length ? 'Secrets in __NEXT_DATA__' : '__NEXT_DATA__ has no secret fields', leaked.length ? 'high' : 'info', leaked.length === 0,
      leaked.length ? `__NEXT_DATA__ contains ${leaked.join(', ')}` : 'no sensitive fields serialized into the page',
      leaked.length ? 'Never put secrets in getServerSideProps/getStaticProps return values — they ship to the browser in __NEXT_DATA__. Use server-only env + API routes.' : undefined));
    return out;
  },
  WordPress: async (ctx) => {
    const out: Finding[] = [];
...
export async function frameworkChecks(ctx: ScanContext): Promise<Finding[]> {
  if (!ctx.res) return [];
  const stacks = ctx.stacks ?? detectStacks({ html: ctx.html, headers: ctx.headers, setCookie: (ctx.headers as any).getSetCookie?.() ?? [] });
  ...
  const out: Finding[] = [f('framework.detected', `Detected: ${high.map((s) => s.name).join(', ')}`, 'info', true, high.map((s) => `${s.name} (${s.why.join('; ')})`).join(' | '))];
  for (const s of high) {
    const runner = CHECKS[s.name];
    if (runner) out.push(...await runner(ctx).catch(() => []));
  }
  return out;
}
