...
// A detector pulls "name @ version" out of a URL or inline banner. `key` is the canonical library name.
interface Detector { key: string; url?: RegExp; banner?: RegExp }
...
export function detectComponents(htmlText: string, baseUrl: string): DetectedComponent[] {
  const urls = [
    ...H.scripts(htmlText).map((s) => resolveUrl(baseUrl, s) || s),
    ...[...htmlText.matchAll(/<link\b[^>]*href=["']([^"']+)["']/gi)].map((m) => resolveUrl(baseUrl, m[1]) || m[1]),
  ];
  const found = new Map<string, DetectedComponent>(); // key name+version → dedup
  for (const det of DETECTORS) {
    for (const u of urls) {
      const m = det.url && u.match(det.url);
      if (m) { const c = { name: det.key, version: m[1], evidence: u.slice(0, 120) }; found.set(`${c.name}@${c.version}`, c); }
    }
    const bm = det.banner && htmlText.match(det.banner);
    if (bm) { const c = { name: det.key, version: bm[1], evidence: `inline banner: ${bm[0].slice(0, 60)}` }; found.set(`${c.name}@${c.version}`, c); }
  }
  return [...found.values()];
}

// Match a detected component against the vuln table; returns the applicable (most severe / lowest-fixed) entry.
export function matchVulnerabilities(c: DetectedComponent): Vuln | null {
...
export async function componentChecks(ctx: ScanContext): Promise<Finding[]> {
  ...
  for (const c of components) {
    ...
    if (v) {
      vulnerable++;
      out.push(f(`components.${c.name}`, `Outdated ${c.name} ${c.version} — ${v.ref}`, v.severity, false,
        `${c.name} ${c.version} is vulnerable (${v.ref}): ${v.note}. [${c.evidence}]`,
        `Upgrade ${c.name} to ${v.below} or later (the first version without this issue).`));
    } else {
  ...
  if (!vulnerable) out.push(f('components.summary', 'All detected client libraries are current', 'info', true, `${components.length} librar${components.length === 1 ? 'y' : 'ies'} detected, none in a known-vulnerable range`, undefined));
  return out;
}
