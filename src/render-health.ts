...
export function renderHealthFindings(path: string, r: RenderHealth, category: Finding['category'] = 'reliability'): Finding[] {
  const mk = (id: string, title: string, severity: Severity, pass: boolean, detail: string, fix?: string): Finding =>
    ({ category, id: `${id}${path}`, title, severity, pass, detail, fix });
  ...
  if (r.errorSignature && r.visibleTextLen > 0 && r.visibleTextLen < 600) {
    return [mk('render.errorboundary', `Client-side crash / error boundary rendered on ${path}`, 'high', false,
      `The page rendered a framework error fallback ("${r.errorSignature}") instead of content — an error boundary caught a thrown exception, so no uncaught error is reported but the page is broken for users.`,
      'Find the component that throws on this route (often unguarded data — an undefined field/array). This class of bug returns HTTP 200 and produces no console pageerror, so only a rendered-state check catches it.')];
  }
  if (r.overlay) {
    return [mk('render.overlay', `Framework error overlay on ${path}`, 'high', false,
      `A framework error overlay (<${r.overlay}>) is present — an unhandled runtime error is being shown over the page.`,
      'Fix the runtime error surfaced by the overlay; ship with error overlays disabled in production.')];
  }
  if (r.blankRoot) {
    return [mk('render.blank', `Blank app root on ${path}`, 'medium', false,
      `The SPA mount ${r.blankRoot} rendered no meaningful content (text length ${r.rootTextLen}, no elements) after the network settled — a white-screen render failure.`,
      'The app mounted but produced no output — usually a crash during initial render or a failed critical data load. Check the console and the route component.')];
  }
