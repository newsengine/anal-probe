...
export function csrfFindings(ctx: ScanContext): Finding[] {
  const html = ctx.html;
  if (!html) return [];
  ...
  if (!postForms.length) return []; // no state-changing HTML forms (SPA/header-token apps) → not applicable
  ...
  if (sameSiteProtected) {
    return [f('csrf', 'POST form has no CSRF token (mitigated by SameSite cookie)', 'low', false,
      `${unprotectedForms.length} POST form(s) lack a CSRF token, but the session cookie is SameSite=${/strict/i.test(sessionCookie!) ? 'Strict' : 'Lax'} which blocks most cross-site submits — add a token for defense-in-depth`,
      'Add a per-request anti-CSRF token (synchronizer token or double-submit cookie) to state-changing forms; SameSite alone is not a complete defense.')];
  }
  return [f('csrf', `${unprotectedForms.length} POST form(s) without CSRF protection`, 'medium', false,
    `state-changing POST form(s) have no anti-CSRF token${sessionCookie ? ' and the session cookie is not SameSite=Lax/Strict' : ''} — another site could submit them on a logged-in victim's behalf`,
    'Add a per-request anti-CSRF token to every state-changing form and verify it server-side; also set SameSite=Lax (or Strict) on session cookies.')];
}
