...
const f = (id, title, severity, pass, detail, fix) => ({ category: 'agent', id, title, severity, pass, detail, fix });
 ...
 */
export function aiCrawlersBlocked(robotsTxt, uaList = AI_CRAWLERS) {
    ...
    for (const rawLine of robotsTxt.split(/\r?\n/)) {
        ...
        if (field === 'user-agent') {
            ...
            if (!current || sawRuleSincePrevAgent) {
                current = { agents: [], blocksRoot: false };
                groups.push(current);
                sawRuleSincePrevAgent = false;
            }
            ...
            if (field === 'disallow' && value === '/')
                current.blocksRoot = true;
        }
    }
    const groupFor = (ua) => groups.find((g) => g.agents.includes(ua)) ?? groups.find((g) => g.agents.includes('*'));
    const blocked = [];
...
export function evaluateAgentReadiness(inp) {
    const out = [];
    ...
    out.push(f('agent.ssr-content', jsOnly ? 'Content is JavaScript-only (agents may see an empty page)' : 'Content is server-rendered', 'medium', !jsOnly, jsOnly ? `only ${text.length} chars of text in the raw HTML — agents/LLMs that don't execute JS see a near-empty shell` : `${text.length} chars of readable text in the initial HTML`, jsOnly ? 'Server-render or pre-render the important content (SSR/SSG) so crawlers and agents can read it without running JavaScript.' : undefined));
    // 2. llms.txt — the emerging standard for agent-facing site guidance.
    out.push(f('agent.llms-txt', inp.hasLlmsTxt ? 'Has /llms.txt' : 'No /llms.txt', 'low', inp.hasLlmsTxt, inp.hasLlmsTxt ? '/llms.txt is served' : 'no /llms.txt found', inp.hasLlmsTxt ? undefined : 'Add a /llms.txt (Markdown) summarising your site + key links so AI agents can navigate it. See llmstxt.org.'));
    ...
    if (inp.robotsPresent) {
        ...
        out.push(f('agent.ai-crawlers', blocked.length ? `robots.txt blocks ${blocked.length} AI crawler(s)` : 'robots.txt allows AI crawlers', 'low', blocked.length === 0, blocked.length ? `blocked at /: ${blocked.slice(0, 8).join(', ')}` : 'no AI crawler is Disallowed from the site root', blocked.length ? 'If you WANT to appear in AI answers, remove the Disallow for these agents. (Ignore this if blocking them is deliberate.)' : undefined));
    }
    else {
        out.push(f('agent.ai-crawlers', 'No robots.txt to declare crawler policy', 'low', false, 'no robots.txt — crawler behaviour is undefined', 'Add a robots.txt so you can explicitly allow (or block) AI crawlers like GPTBot/ClaudeBot.'));
    }
    ...
    out.push(f('agent.structured-data', hasJsonLd ? 'Has JSON-LD structured data' : 'No JSON-LD structured data', 'low', hasJsonLd, hasJsonLd ? 'schema.org JSON-LD present' : 'no <script type="application/ld+json">', hasJsonLd ? undefined : 'Add schema.org JSON-LD (Organization/Article/Product…) so agents can extract structured facts about your content.'));
    // 5. Agent/MCP manifest discovery — a bonus, purely informational.
    out.push(f('agent.manifest', inp.hasAgentManifest ? 'Has an agent/MCP manifest' : 'No agent/MCP manifest', 'info', inp.hasAgentManifest, inp.hasAgentManifest ? 'a /.well-known agent manifest is discoverable' : 'no /.well-known/mcp or ai-plugin manifest (optional)', inp.hasAgentManifest ? undefined : 'Optional: expose a /.well-known/ai-plugin.json or MCP manifest if you want agents to call your APIs directly.'));
    return out;
}
