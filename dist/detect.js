...
export function detectStacks(inp) {
    const h = (n) => inp.headers.get(n) || '';
    const out = [];
    for (const rule of RULES) {
        const hit = rule.signals.filter((s) => { try {
            return s.test(inp, h);
        }
        catch {
            return false;
        ...
        const confidence = hit.some((s) => s.strong) || hit.length >= 2 ? 'high' : 'medium';
