...
const f = (id, title, severity, pass, detail, fix) => ({ category: 'reliability', id, title, severity, pass, detail, fix });
...
export async function browseChecks(startUrl, opts = {}) {
    let chromium;
    try {
    ...
    try {
        ...
        try {
            ...
            return [f('browse.no-browser', 'no browser available', 'info', true, 'install Chrome, or run `npx playwright install chromium`')];
        }
    }
    ...
    if (opts.headers) {
        ...
        for (const [k, v] of Object.entries(opts.headers)) {
            ...
            if (key === 'cf-access-client-id' || key === 'cf-access-client-secret' || key === 'x-smoke-key')
                continue;
            safe[k] = v;
        }
        if (Object.keys(safe).length)
            await ctx.setExtraHTTPHeaders(safe);
    }
    ...
    for (const url of pages) {
        ...
        page.on('response', (r) => { const s = r.status(); if (s >= 400 && r.request().resourceType() !== 'document')
            failedResponses.push(`${s} ${r.url().slice(0, 100)}`); });
        const path = (() => { try {
        ...
        if (consoleErrors.length)
            out.push(f(`browse.console${path}`, `${consoleErrors.length} console error(s) on ${path}`, 'low', false, consoleErrors.slice(0, 3).join(' | '), 'Investigate console errors — often failed fetches or missing assets.'));
        // Failed subresources (images/scripts/styles/XHR returning 4xx/5xx).
        if (failedResponses.length)
            out.push(f(`browse.failed-requests${path}`, `${failedResponses.length} failed request(s) on ${path}`, 'medium', false, failedResponses.slice(0, 5).join(', '), 'Fix or remove resources that 4xx/5xx — they break rendering and features.'));
        // Broken images (rendered, naturalWidth 0).
        if (probe.brokenImages.length)
            out.push(f(`browse.images${path}`, `${probe.brokenImages.length} broken image(s) on ${path}`, 'medium', false, probe.brokenImages.slice(0, 4).join(', '), 'Fix the image src / missing files — they show as broken in the browser.'));
