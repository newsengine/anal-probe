...
function chromeBin() { return CHROME_CANDIDATES.find((p) => { try {
...
export async function launchProfile(opts) {
    ...
    try {
        ...
        throw new Error('browser auth needs playwright-core — run: npm i -D playwright-core');
    }
    const bin = chromeBin();
    if (!bin)
        throw new Error('Google Chrome not found — install it or set CHROME_BIN');
    ...
    for (let i = 0; i < 40; i++) {
        try {
            if ((await fetch(`http://localhost:${opts.port}/json/version`)).ok)
                break;
        }
        catch { }
