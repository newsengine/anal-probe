...
export function issueKey(target, finding) {
    let host = target;
    try {
...
const MARK_CLOSE = '-->';
export function markerFor(key) { return `${MARK_OPEN} ${key} ${MARK_CLOSE}`; }
export function keyFromBody(body) {
    const m = body.match(/<!--\s*vta-key:\s*(\S+)\s*-->/);
    return m ? m[1] : null;
}
const SEV_LABEL = { high: 'severity:high', medium: 'severity:medium', low: 'severity:low', info: 'severity:info' };
function bodyFor(target, f, key) {
    const spec = catalogEntryFor(f.id);
    const refs = refsFor(f.id);
    const std = [];
    if (refs.owasp)
        std.push(`OWASP ${refs.owasp}`);
    if (refs.apiTop10)
        std.push(refs.apiTop10);
    if (refs.cwe?.length)
        std.push(refs.cwe.join(' '));
    if (spec?.atlas?.length)
        std.push(`ATLAS ${spec.atlas.join(' ')}`);
    return [
        markerFor(key),
 ...
 */
export function planIssueActions(findings, opts) {
    ...
    for (const f of failing) {
        ...
        if (failingKeys.has(key))
            continue; // dedupe within a single scan
        failingKeys.add(key);
        const spec = { key, title: titleFor(opts.target, f), body: bodyFor(opts.target, f, key), labels: ['vta', `vta:${f.category}`, SEV_LABEL[f.severity]] };
        const prior = byKey.get(key);
        if (prior)
            toUpdate.push({ number: prior.number, spec });
        else
            toOpen.push(spec);
    }
...
export function summarizeActions(a) {
