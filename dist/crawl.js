...
const f = (id, title, severity, detail, fix, category = 'reliability') => ({ category, id, title, severity, pass: false, detail, fix });
export async function crawlAudit(startUrl, opts = {}) {
    const origin = new URL(startUrl).origin;
    const maxPages = opts.maxPages ?? 30;
    const timeout = opts.timeoutMs ?? 25000;
    const findings = [];
    const sameOrigin = (u) => { try {
    ...
    let browser, closeSession = null, chromium;
    try {
    ...
    const seen = new Set();
    const queue = [];
    for (const p of (opts.startPaths ?? ['/'])) {
        const n = norm(origin + p);
        if (n)
            queue.push(n);
    }
    while (queue.length && seen.size < maxPages) {
        ...
        const jsErr = [], conErr = [], failed = [];
        const onErr = (e) => jsErr.push(String(e?.message || e).slice(0, 140));
        const onCon = (m) => { if (m.type() === 'error')
            conErr.push(m.text().slice(0, 140)); };
        const onResp = (r) => { try {
            const st = r.status();
            if (st >= 400 && r.request().resourceType() !== 'document')
                failed.push(`${st} ${r.url().replace(origin, '').slice(0, 70)}`);
        }
        catch { } };
        ...
        try {
            ...
            if (resp && resp.status() >= 400)
                findings.push(f(`crawl.status${path}`, `Page ${path} returns ${resp.status()}`, 'medium', `authenticated crawl: ${resp.status()} on a navigable route`, 'Fix or remove the broken route/link.'));
        }
        catch (e) {
        ...
        if (failed.length)
            findings.push(f(`crawl.req${path}`, `Failed request(s) on ${path}`, 'medium', [...new Set(failed)].slice(0, 4).join(', '), 'Fix resources/APIs returning 4xx/5xx on this page.'));
        if (conErr.length)
            findings.push(f(`crawl.con${path}`, `Console error(s) on ${path}`, 'low', conErr.slice(0, 2).join(' | '), 'Investigate console errors — often failed fetches or feature bugs.'));
        const probe = await page.evaluate(() => {
            ...
            return { brokenImg, searchInputs, links: Array.from(document.querySelectorAll('a[href]')).map((a) => a.href) };
        });
        if (probe.brokenImg.length)
            findings.push(f(`crawl.img${path}`, `Broken image(s) on ${path}`, 'medium', probe.brokenImg.slice(0, 3).join(', '), 'Fix the missing image sources.'));
        for (const href of probe.links) {
            const n = norm(href);
            if (n && sameOrigin(n) && !seen.has(n) && !queue.includes(n))
                queue.push(n);
        }
        ...
        if (probe.searchInputs > 0) {
            for (const payload of [XSS_IN, SQLI_IN]) {
                try {
                    ...
                    if (payload === XSS_IN && html.includes(`<${MARK}>`))
                        findings.push(f(`crawl.xss${path}`, `Reflected HTML injection on ${path}`, 'high', `search reflected "<${MARK}>" UNESCAPED`, 'Encode user input before rendering it — reflected XSS risk.', 'security'));
                    if (payload === SQLI_IN && SQL_ERR.test(html))
                        findings.push(f(`crawl.sqli${path}`, `SQL error surfaced on ${path}`, 'high', 'a search input produced a SQL error string', 'Use parameterised queries; never build SQL from user input.', 'security'));
    ...
    if (closeSession)
        await closeSession();
    findings.pagesVisited = [...seen].map((u) => new URL(u).pathname);
    return findings;
}
