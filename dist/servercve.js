...
const lt = (v, b) => { const a = parseVersion(v), c = parseVersion(b); for (let i = 0; i < 3; i++) {
    if ((a[i] || 0) !== (c[i] || 0))
        return (a[i] || 0) < (c[i] || 0);
} return false; };
const between = (v, lo, hi) => !lt(v, lo) && lt(v, hi); // [lo, hi)
// Curated high-signal server/runtime CVEs that matter from a black-box banner. Kept small + exact.
export const SERVER_CVES = [
