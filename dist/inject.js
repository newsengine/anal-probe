...
// ── detectors (pure) ────────────────────────────────────────────────────────────────────────────────
export async function detectSqlError(baseline, prober) {
...
export async function detectSqlBoolean(baseline, prober) {
    const truthy = await prober("' AND '1'='1");
    const falsy = await prober("' AND '1'='2");
    if (SQL_ERROR.test(truthy.body) || SQL_ERROR.test(falsy.body))
        return null; // error-based handles this
...
export async function detectSqlTime(baseline, prober) {
...
export async function detectSsti(prober) {
    const marker = 'vtaSS';
...
export async function detectCmdTime(baseline, prober) {
...
export async function detectTraversal(baseline, prober) {
    if (PASSWD.test(baseline.body))
        return null;
...
export async function runDetectors(baseline, prober) {
    const hits = [];
    const err = await detectSqlError(baseline, prober);
    if (err)
        hits.push(err);
    if (!err) {
        const b = await detectSqlBoolean(baseline, prober);
        if (b)
            hits.push(b);
    }
    const t = await detectSqlTime(baseline, prober);
    if (t && !hits.some((h) => h.kind === 'sqli'))
        hits.push(t);
    const s = await detectSsti(prober);
    if (s)
        hits.push(s);
    const c = await detectCmdTime(baseline, prober);
    if (c)
        hits.push(c);
    const tr = await detectTraversal(baseline, prober);
    if (tr)
        hits.push(tr);
    return hits;
}
 ...
 */
export async function activeInjectionScan(ctx) {
    if (!ctx.opts.authorizedActive)
        return [];
    const out = [];
    ...
    const urls = new Set(map.paramUrls);
    try {
        if (new URL(ctx.baseUrl).search)
    ...
    for (const u of urls) {
        ...
        for (const param of parsed.searchParams.keys())
            targets.push({ url: u, param });
        if (targets.length >= 40)
            break; // bound the active surface
    }
    ...
    for (const { url, param } of targets.slice(0, 40)) {
        ...
        for (const h of hits) {
            ...
            out.push(f(`inject.${h.kind}.${param}`, `${meta.title} via ?${param}`, h.severity, false, `[${new URL(url).pathname}] ${h.detail} — ${h.evidence}`, meta.fix));
        }
    }
    if (!out.length)
        out.push(f('inject.none', 'No injection found on tested parameters', 'info', true, `tested ${tested.size} parameter(s) with SQLi/SSTI/cmd/traversal probes`));
    return out;
}
