...
export function scanJsonLdBreakout(html) {
    const issues = [];
...
export function jsonLdFindings(ctx) {
    if (!ctx.html)
        return [];
    ...
    return issues.map((it) => f(it.index === 1 ? 'xss.jsonld' : `xss.jsonld.${it.index}`, it.severity === 'high' ? 'JSON-LD script breakout (stored XSS)' : 'JSON-LD not <-escaped (stored-XSS risk)', it.severity, false, it.detail, FIX));
}
