...
const f = (id, title, severity, pass, detail, fix) => ({ category: 'host', id, title, severity, pass, detail, fix });
...
export function identifyEdge(headers) {
    const h = (n) => (headers instanceof Headers ? headers.get(n) : (headers[n] ?? headers[n.toLowerCase()])) || '';
    const matched = EDGE_SIGS.filter((s) => { try {
        return s.test(h);
    }
    catch {
        return false;
...
export async function hostChecks(ctx) {
    if (!ctx.res)
        return [];
    const out = [];
    ...
    if (a[0]) {
        const ptr = await dns.reverse(a[0]).catch(() => []);
        if (ptr.length)
            out.push(f('host.ptr', 'Reverse DNS', 'info', true, `${a[0]} → ${ptr.slice(0, 3).join(', ')}`));
    }
